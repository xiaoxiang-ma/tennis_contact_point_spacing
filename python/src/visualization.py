"""Visualization: skeleton overlay, contact point marker, debug frame annotations."""
...
def annotate_contact_frame(frame: np.ndarray,
                           ...
                           fps: float,
                           confidence: float,
                           label: str = "CONTACT") -> np.ndarray:
    """Annotate a frame as a detected contact with a red border and label.
    ...
    # Info panel at top
    timestamp = frame_num / fps if fps > 0 else 0
    text = f"{label} | Frame {frame_num} | {timestamp:.2f}s | Conf: {confidence:.0%}"

    _draw_text(annotated, text, (10, 30), color=(0, 0, 255), scale=0.7)
